...
window.QLogFeatures = (function() {
    ...
    const BADGES = [
        ...
        { id: 'nightOwl',   emoji: '🦉', name: '야행성 작가',   desc: '자정이 넘어 일기를 썼어요',        check: (l, s) => l.some(x => { const h = new Date(x.date).getHours(); return h >= 0 && h < 4; }) },
        { id: 'earlyBird',  emoji: '🐦', name: '얼리버드',      desc: '오전 6시 이전에 일기를 썼어요',   check: (l, s) => l.some(x => new Date(x.date).getHours() < 6) },
